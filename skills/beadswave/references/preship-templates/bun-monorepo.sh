#!/usr/bin/env bash
# .beadswave/pre-ship.sh — Bun workspace / monorepo starter template.
#
# Mirrors the most common CI job set for Bun + TS projects. Customize by:
...
echo "▶ Pre-ship gates (bun workspace)"

beadswave_run_gate "lint (all workspaces)"      "bun run --filter '*' lint" || exit 1
beadswave_run_gate "typecheck (all workspaces)" "bun run --filter '*' typecheck" || exit 1
beadswave_run_gate "bun audit"                  "bun audit" || exit 1
beadswave_run_gate "tests (all workspaces)"     "bun run --filter '*' test" || exit 1
beadswave_run_gate "build (all workspaces)"     "bun run --filter '*' build" || exit 1
