...
osascript -e "display notification \"Session ending. Duration: ${DURATION} | ${CONTEXT}\" with title \"Claude Session Ending\" subtitle \"Repo: ${REPO}/${BRANCH}\" sound name \"Purr\""

# SEND TO IPHONE (update the phone number!)
MSG="Claude session ending in ${REPO}/${BRANCH} - Duration: ${DURATION} - ${CONTEXT}"
osascript -e 'tell application "Messages"' -e "send \"${MSG}\" to buddy \"+919766257263\" of (1st service whose service type = iMessage)" -e 'end tell'
